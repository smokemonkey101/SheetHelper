...
const maxFileBytes = (Number.isFinite(configuredMaxUploadMb) && configuredMaxUploadMb > 0
...
function signedJwt(email, key, scopes) {
...
async function googleToken(settings, scopes) {
...
function requireGoogleTargets(settings) {
    if (!settings.spreadsheetId)
        throw new Error("Spreadsheet ID is not configured.");
}
async function ensureWorkbook(settings, token) {
    ...
    for (const [title, oldHeaders] of Object.entries(previousSheetDefinitions)) {
        ...
        if (sheetId === undefined)
            continue;
        const current = await googleJson(`https://sheets.googleapis.com/v4/spreadsheets/${id}/values/${encodeURIComponent(`${title}!1:1`)}`, token);
...
async function getJobs(settings) {
    ...
    const result = await googleJson(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(settings.spreadsheetId)}/values/${range}`, token);
    return [...new Set((result.values || []).map((row) => clean(row[0])).filter(Boolean))]
        .sort((a, b) => a.localeCompare(b));
}
async function validateJob(settings, job) {
...
function photoFileName(job, date, index = 1) {
...
async function uploadDriveFile(settings, file, fileName, mimeType = "application/octet-stream") {
    ...
    if (file.length > multipartUploadMaxBytes) {
        const sessionResponse = await fetch(`${uploadUrl}?uploadType=resumable&supportsAllDrives=true&fields=id,name,webViewLink`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${token}`,
        ...
        response = await fetch(`${uploadUrl}?uploadType=multipart&supportsAllDrives=true&fields=id,name,webViewLink`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${token}`,
...
async function readReceipt(settings, image) {
    const token = await googleToken(settings, ["https://www.googleapis.com/auth/cloud-platform"]);
    const result = await googleJson("https://vision.googleapis.com/v1/images:annotate", token, {
        method: "POST",
        body: {
            requests: [{
...
async function api(req, res, route) {
    ...
    if (route === "/api/config" && req.method === "GET") {
        ...
        sendJson(res, 200, {
            ...
            railwayConfiguration: {
                ...
                serviceAccount: Boolean(clean(process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL) && clean(process.env.GOOGLE_PRIVATE_KEY)),
                oauthClient: Boolean(clean(process.env.GOOGLE_OAUTH_CLIENT_ID) && clean(process.env.GOOGLE_OAUTH_CLIENT_SECRET)),
    ...
    if (route === "/api/reports" && req.method === "POST") {
        ...
        if (!job || !report)
            throw new Error("Choose a job and enter a report.");
...
if (require.main === module) {
    http.createServer((req, res) => {
        ...
        if (route.startsWith("/api/")) {
            void api(req, res, route).catch((error) => {
